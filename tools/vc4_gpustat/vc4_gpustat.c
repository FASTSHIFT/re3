...
init_v3d(void)
{
	...
	void *m = mmap(NULL, V3D_MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, V3D_PHYS_BASE);
	close(fd);
	if(m == MAP_FAILED) {
		perror("mmap V3D");
		return -1;
...
setup_counters(void)
{
	...
	/* Configure sources */
	for(int i = 0; i < NCTR; i++) wr(OFF_PCTRS(i), ctrs[i].src);
	/* Enable counters (bit 31 = EN, bits 0..15 = enable mask) */
	wr(OFF_PCTRE, 0x80000000u | ((1u << NCTR) - 1));
}

static void
read_counters(uint32_t *out)
{
	for(int i = 0; i < NCTR; i++) out[i] = rd(OFF_PCTR(i));
}
...
static void
print_report(const uint32_t *v0, const uint32_t *v1, uint32_t bfc0, uint32_t bfc1, uint32_t rfc0, uint32_t rfc1, double dt, int sample)
{
	/* Counters wrap at 2^32, delta handles wrap naturally for uint32 */
	uint32_t d[NCTR];
	for(int i = 0; i < NCTR; i++) d[i] = v1[i] - v0[i];
...
#define FEP_PRIMS d[12]
	...
	uint64_t qpu_total = (uint64_t)QPU_IDLE + qpu_active + QPU_TMU_ST + QPU_SB_ST;
