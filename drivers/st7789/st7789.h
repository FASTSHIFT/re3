...
typedef enum { ST7789_ROTATE_0 = 0, ST7789_ROTATE_90 = 1, ST7789_ROTATE_180 = 2, ST7789_ROTATE_270 = 3 } st7789_rotation_t;
...
void
st7789_close(st7789_t *dev);

/* Effective resolution (after rotation); the caller allocates the framebuffer accordingly. */
int
st7789_get_width(const st7789_t *dev);
int
st7789_get_height(const st7789_t *dev);
 ...
 */
int
st7789_flush(st7789_t *dev, const uint16_t *rgb565);
 ...
 */
int
st7789_flush_area(st7789_t *dev, int x, int y, int w, int h, const uint16_t *rgb565);

/* Fill the whole screen with a solid color. */
int
st7789_fill(st7789_t *dev, uint16_t color);

/* Toggle backlight (requires blk_pin to be configured). */
void
st7789_set_backlight(st7789_t *dev, int on);

/* RGB888 -> RGB565 helper for upper layers. */
static inline uint16_t
st7789_rgb565(uint8_t r, uint8_t g, uint8_t b)
{
