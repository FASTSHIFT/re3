...
static int spi_write(st7789_t* d, const uint8_t* buf, size_t len)
{
    ...
    while (off < len) {
        ...
        if (ioctl(d->spi_fd, SPI_IOC_MESSAGE(1), &tr) < 1)
            return -1;
        off += n;
    }
    return 0;
...
static int write_cmd(st7789_t* d, uint8_t cmd)
{
...
static int write_data(st7789_t* d, const uint8_t* data, size_t len)
{
    pi_gpio_set_value(d->cs_pin, 0);
...
static int write_data8(st7789_t* d, uint8_t b)
{
    return write_data(d, &b, 1);
}

static uint8_t madctl_for_rotation(st7789_rotation_t r)
{
    switch (r) {
    case ST7789_ROTATE_0:
        return MADCTL_RGB;
    case ST7789_ROTATE_90:
        return (MADCTL_MV | MADCTL_MY) | MADCTL_RGB;
    case ST7789_ROTATE_180:
        return (MADCTL_MX | MADCTL_MY) | MADCTL_RGB;
    case ST7789_ROTATE_270:
        return (MADCTL_MV | MADCTL_MX) | MADCTL_RGB;
    default:
        return MADCTL_RGB;
    }
}
...
static int set_window(st7789_t* d, int x0, int y0, int x1, int y1)
{
    x0 += d->x_offset;
    x1 += d->x_offset;
    y0 += d->y_offset;
    ...
    if (write_cmd(d, ST_CMD_CASET) < 0)
        return -1;
    buf[0] = (uint8_t)(x0 >> 8);
    buf[1] = (uint8_t)x0;
    buf[2] = (uint8_t)(x1 >> 8);
    buf[3] = (uint8_t)x1;
    if (write_data(d, buf, 4) < 0)
        return -1;

    if (write_cmd(d, ST_CMD_RASET) < 0)
        return -1;
    buf[0] = (uint8_t)(y0 >> 8);
    buf[1] = (uint8_t)y0;
    buf[2] = (uint8_t)(y1 >> 8);
...
static int panel_init(st7789_t* d, int invert)
{
    if (d->rst_pin >= 0) {
        ...
        if (write_cmd(d, ST_CMD_SWRESET) < 0)
            return -1;
        delay_ms(150);
    }
    ...
    if (write_cmd(d, ST_CMD_MADCTL) < 0)
        return -1;
    if (write_data8(d, madctl_for_rotation(d->rotation)) < 0)
        return -1;

    if (write_cmd(d, ST_CMD_COLMOD) < 0) /* 16-bit/pixel */
        return -1;
    if (write_data8(d, 0x05) < 0)
        return -1;

    if (d->little_endian) {
        /* RAMCTRL: set little-endian so RGB565 bytes go out LSB-first */
        uint8_t rc[2] = { 0x00, 0xF8 };
        if (write_cmd(d, ST_CMD_RAMCTRL) < 0)
            return -1;
        if (write_data(d, rc, 2) < 0)
            return -1;
    }
    ...
    {
        static const uint8_t gpos[] = { 0xD0, 0x08, 0x0E, 0x09, 0x09, 0x05,
            0x31, 0x33, 0x48, 0x17, 0x14, 0x15, 0x31, 0x34 };
        static const uint8_t gneg[] = { 0xD0, 0x08, 0x0E, 0x09, 0x09, 0x15,
            0x31, 0x33, 0x48, 0x17, 0x14, 0x15, 0x31, 0x34 };
        write_cmd(d, 0xE0);
    ...
    if (write_cmd(d, invert ? ST_CMD_INVON : ST_CMD_INVOFF) < 0)
        return -1;
    if (write_cmd(d, ST_CMD_DISPON) < 0)
        return -1;
...
st7789_t* st7789_open(const st7789_config_t* cfg)
{
    ...
    pi_gpio_set_value(d->cs_pin, 1);
    if (d->rst_pin >= 0)
        pi_gpio_set_mode(d->rst_pin, PI_GPIO_OUTPUT);
    if (d->blk_pin >= 0) {
...
void st7789_close(st7789_t* d)
{
    if (!d)
        return;
    if (d->blk_pin >= 0)
        pi_gpio_set_value(d->blk_pin, 0);
    if (d->spi_fd >= 0)
        close(d->spi_fd);
    free(d->linebuf);
    free(d);
...
int st7789_get_width(const st7789_t* d) { return d ? d->width : 0; }
int st7789_get_height(const st7789_t* d) { return d ? d->height : 0; }

int st7789_flush(st7789_t* d, const uint16_t* rgb565)
{
    if (!d || !rgb565)
        return -1;
...
int st7789_flush_area(st7789_t* d, int x, int y, int w, int h, const uint16_t* rgb565)
{
    if (!d || !rgb565 || w <= 0 || h <= 0)
        return -1;
    if (x < 0 || y < 0 || x + w > d->width || y + h > d->height)
        return -1;
    if (set_window(d, x, y, x + w - 1, y + h - 1) < 0)
        return -1;
    return write_data(d, (const uint8_t*)rgb565, (size_t)w * h * 2);
}

int st7789_fill(st7789_t* d, uint16_t color)
{
    if (!d)
        return -1;
    if (set_window(d, 0, 0, d->width - 1, d->height - 1) < 0)
...
void st7789_set_backlight(st7789_t* d, int on)
{
    if (d && d->blk_pin >= 0)
        pi_gpio_set_value(d->blk_pin, on ? 1 : 0);
}
...
int st7789_tune_system(uint32_t want_bufsiz)
{
    ...
    for (int cpu = 0;; cpu++) {
        ...
        if (!f) {
            if (cpu == 0)
                rc = -1; /* couldn't set even cpu0 */
            break;       /* no more CPUs */
        }
        if (fputs("performance", f) < 0)
            rc = -1;
        fclose(f);
    }
