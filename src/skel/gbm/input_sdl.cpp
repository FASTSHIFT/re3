...
build_keymap(void)
{
	for(int i = 0; i < SDL_NUM_SCANCODES; i++) sKeymap[i] = 0;

	for(int c = 0; c < 26; c++) sKeymap[SDL_SCANCODE_A + c] = 'A' + c;
	sKeymap[SDL_SCANCODE_1] = '1';
	sKeymap[SDL_SCANCODE_2] = '2';
	sKeymap[SDL_SCANCODE_3] = '3';
	sKeymap[SDL_SCANCODE_4] = '4';
	sKeymap[SDL_SCANCODE_5] = '5';
	sKeymap[SDL_SCANCODE_6] = '6';
	sKeymap[SDL_SCANCODE_7] = '7';
	sKeymap[SDL_SCANCODE_8] = '8';
	sKeymap[SDL_SCANCODE_9] = '9';
	sKeymap[SDL_SCANCODE_0] = '0';

	sKeymap[SDL_SCANCODE_SPACE] = ' ';
	sKeymap[SDL_SCANCODE_APOSTROPHE] = '\'';
	sKeymap[SDL_SCANCODE_COMMA] = ',';
	sKeymap[SDL_SCANCODE_MINUS] = '-';
	sKeymap[SDL_SCANCODE_PERIOD] = '.';
	sKeymap[SDL_SCANCODE_SLASH] = '/';
	sKeymap[SDL_SCANCODE_SEMICOLON] = ';';
	sKeymap[SDL_SCANCODE_EQUALS] = '=';
	sKeymap[SDL_SCANCODE_LEFTBRACKET] = '[';
	sKeymap[SDL_SCANCODE_BACKSLASH] = '\\';
	sKeymap[SDL_SCANCODE_RIGHTBRACKET] = ']';
	sKeymap[SDL_SCANCODE_GRAVE] = '`';

	sKeymap[SDL_SCANCODE_ESCAPE] = rsESC;
	sKeymap[SDL_SCANCODE_RETURN] = rsENTER;
	sKeymap[SDL_SCANCODE_TAB] = rsTAB;
	sKeymap[SDL_SCANCODE_BACKSPACE] = rsBACKSP;
	sKeymap[SDL_SCANCODE_INSERT] = rsINS;
	sKeymap[SDL_SCANCODE_DELETE] = rsDEL;
	sKeymap[SDL_SCANCODE_RIGHT] = rsRIGHT;
	sKeymap[SDL_SCANCODE_LEFT] = rsLEFT;
	sKeymap[SDL_SCANCODE_DOWN] = rsDOWN;
	sKeymap[SDL_SCANCODE_UP] = rsUP;
	sKeymap[SDL_SCANCODE_PAGEUP] = rsPGUP;
	sKeymap[SDL_SCANCODE_PAGEDOWN] = rsPGDN;
	sKeymap[SDL_SCANCODE_HOME] = rsHOME;
	sKeymap[SDL_SCANCODE_END] = rsEND;
	sKeymap[SDL_SCANCODE_CAPSLOCK] = rsCAPSLK;
	sKeymap[SDL_SCANCODE_PAUSE] = rsPAUSE;

	sKeymap[SDL_SCANCODE_F1] = rsF1;
	sKeymap[SDL_SCANCODE_F2] = rsF2;
	sKeymap[SDL_SCANCODE_F3] = rsF3;
	sKeymap[SDL_SCANCODE_F4] = rsF4;
	sKeymap[SDL_SCANCODE_F5] = rsF5;
	sKeymap[SDL_SCANCODE_F6] = rsF6;
	sKeymap[SDL_SCANCODE_F7] = rsF7;
	sKeymap[SDL_SCANCODE_F8] = rsF8;
	sKeymap[SDL_SCANCODE_F9] = rsF9;
	sKeymap[SDL_SCANCODE_F10] = rsF10;
	sKeymap[SDL_SCANCODE_F11] = rsF11;
	sKeymap[SDL_SCANCODE_F12] = rsF12;

	sKeymap[SDL_SCANCODE_LSHIFT] = rsLSHIFT;
	sKeymap[SDL_SCANCODE_RSHIFT] = rsRSHIFT;
	sKeymap[SDL_SCANCODE_LCTRL] = rsLCTRL;
	sKeymap[SDL_SCANCODE_RCTRL] = rsRCTRL;
	sKeymap[SDL_SCANCODE_LALT] = rsLALT;
	sKeymap[SDL_SCANCODE_RALT] = rsRALT;
}
...
sdlin_poll(void)
{
	...
	for(int i = 0; i < n; i++) {
		...
		if(sc >= SDL_NUM_SCANCODES) continue;
		int rs = sKeymap[sc];
		if(rs == 0) continue;
		if(evs[i].type == SDL_KEYDOWN) {
			if(evs[i].key.repeat == 0) RsKeyboardEventHandler(rsKEYDOWN, &rs);
		} else {
	...
	int buttons = ((btn & SDL_BUTTON(SDL_BUTTON_LEFT)) ? (1 << GLFW_MOUSE_BUTTON_LEFT) : 0) |
