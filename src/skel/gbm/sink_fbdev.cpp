...
fbdev_init(int renderW, int renderH)
{
	...
	if(ioctl(sFd, FBIOGET_VSCREENINFO, &sVar) != 0 || ioctl(sFd, FBIOGET_FSCREENINFO, &sFix) != 0) {
		printf("fbdev: ioctl failed\n");
		close(sFd);
		sFd = -1;
		return true;
	}
	printf("fbdev: /dev/fb0 %dx%d %dbpp line=%d\n", sVar.xres, sVar.yres, sVar.bits_per_pixel, sFix.line_length);
...
fbdev_present(const uint16_t *rgb565, int w, int h)
{
	...
	if(sMem == 0) return; // no display; readback already exercised the pipeline
...
{
	if(sMem != 0 && sSize != 0) munmap(sMem, sSize);
	if(sFd >= 0) close(sFd);
	sMem = 0;
	sFd = -1;
	sSize = 0;
