...
sdl_init(int renderW, int renderH)
{
	...
	sW = renderW; sH = renderH;
	sWin = SDL_CreateWindow("re3 (GBM/SDL debug)",
		SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
		renderW * sScale, renderH * sScale, SDL_WINDOW_SHOWN);
	if (sWin == 0) {
	...
	sTex = SDL_CreateTexture(sRen, SDL_PIXELFORMAT_RGB888,
...
sdl_terminate(void)
{
	if (sRGBX) { free(sRGBX); sRGBX = 0; }
	if (sTex) { SDL_DestroyTexture(sTex); sTex = 0; }
	if (sRen) { SDL_DestroyRenderer(sRen); sRen = 0; }
	if (sWin) { SDL_DestroyWindow(sWin); sWin = 0; }
